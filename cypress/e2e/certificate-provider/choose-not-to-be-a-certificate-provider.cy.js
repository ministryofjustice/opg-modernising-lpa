...
describe('Choose not to be a certificate provider', () => {
    describe('can enter reference number to not be a certificate provider', () => {
        it('when LPA has been signed and witnessed', () => {
            ...
            cy.visit(`/fixtures/certificate-provider?redirect=/certificate-provider-enter-access-code-opt-out&withShareCode=${shareCode}&email=${TestEmail}&progress=signedByDonor`)
        ...
        it('when LPA has not been signed and witnessed', () => {
            const shareCode = randomShareCode()
            cy.visit(`/fixtures/certificate-provider?redirect=/certificate-provider-enter-access-code-opt-out&withShareCode=${shareCode}&email=${TestEmail}`)
