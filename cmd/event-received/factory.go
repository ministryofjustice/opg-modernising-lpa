...
func (f *Factory) ShareCodeSender(ctx context.Context) (ShareCodeSender, error) {
	if f.shareCodeSender == nil {
		...
		f.shareCodeSender = sharecode.NewSender(sharecode.NewStore(f.dynamoClient), notifyClient, f.appPublicURL, event.NewClient(f.cfg, f.eventBusName), certificateprovider.NewStore(f.dynamoClient))
	}
