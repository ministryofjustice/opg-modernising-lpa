...
func TestHandleChangeConfirmedWhenIdentityTaskNotPending(t *testing.T) {
	...
	for actorType, tc := range testcases {
		t.Run(actorType, func(t *testing.T) {
			...
			err := handleChangeConfirmed(ctx, tc.setupDynamoClient(), event, testNowFn, nil, false)
...
func TestHandleChangeConfirmedWhenLpaStoreClientError(t *testing.T) {
	...
	for actorType, tc := range testcases {
		t.Run(actorType, func(t *testing.T) {
			...
			err := handleChangeConfirmed(ctx, tc.setupDynamoClient(t), event, testNowFn, tc.setupLpaStoreClient(t), false)
...
func TestHandleImmaterialChangeConfirmedWhenUnexpectedActorType(t *testing.T) {
	...
	err := handleChangeConfirmed(ctx, nil, event, testNowFn, nil, false)
...
func TestHandleMaterialChangeConfirmed(t *testing.T) {
	testcases := map[string]struct {
		setupDynamoClient   func() *mockDynamodbClient
		setupLpaStoreClient func() *mockLpaStoreClient
	}{
		"donor": {
			setupDynamoClient: func() *mockDynamodbClient {
				updated := &donordata.Provided{
	...
	for actorType, tc := range testcases {
		t.Run(actorType, func(t *testing.T) {
			...
			factory.EXPECT().
				LpaStoreClient().
				Return(tc.setupLpaStoreClient(), nil)
			factory.EXPECT().
				Now().
				Return(testNowFn)
