...
func TestHandleFeeApproved(t *testing.T) {
	...
	now := time.Now()
	updated := &actor.DonorProvidedDetails{PK: "LPA#123", SK: dynamo.DonorKey("456"), Tasks: actor.DonorTasks{PayForLpa: actor.PaymentTaskCompleted}, UpdatedAt: now}
	...
	client.
		On("OneByUID", ctx, "M-1111-2222-3333", mock.Anything).
		Return(func(ctx context.Context, uid string, v interface{}) error {
			b, _ := json.Marshal(dynamo.Key{PK: "LPA#123", SK: dynamo.DonorKey("456")})
			json.Unmarshal(b, v)
			return nil
		})
	client.
		On("One", ctx, "LPA#123", dynamo.DonorKey("456"), mock.Anything).
		Return(func(ctx context.Context, pk, sk string, v interface{}) error {
			b, _ := json.Marshal(actor.DonorProvidedDetails{PK: "LPA#123", SK: dynamo.DonorKey("456"), Tasks: actor.DonorTasks{PayForLpa: actor.PaymentTaskPending}})
			json.Unmarshal(b, v)
			return nil
		})
	client.EXPECT().
...
func TestHandleFeeApprovedWhenDynamoClientPutError(t *testing.T) {
	...
	client.
		On("OneByUID", ctx, "M-1111-2222-3333", mock.Anything).
		Return(func(ctx context.Context, uid string, v interface{}) error {
			b, _ := json.Marshal(actor.DonorProvidedDetails{PK: "LPA#123", SK: dynamo.DonorKey("456")})
			json.Unmarshal(b, v)
			return nil
		})
	client.
...
func TestHandleMoreEvidenceRequired(t *testing.T) {
	...
	now := time.Now()
	updated := &actor.DonorProvidedDetails{PK: "LPA#123", SK: dynamo.DonorKey("456"), Tasks: actor.DonorTasks{PayForLpa: actor.PaymentTaskMoreEvidenceRequired}, UpdatedAt: now}
	updated.Hash, _ = updated.GenerateHash()
...
func TestHandleFeeDenied(t *testing.T) {
	...
	now := time.Now()
	updated := &actor.DonorProvidedDetails{PK: "LPA#123", SK: dynamo.DonorKey("456"), Tasks: actor.DonorTasks{PayForLpa: actor.PaymentTaskDenied}, UpdatedAt: now}
	updated.Hash, _ = updated.GenerateHash()
