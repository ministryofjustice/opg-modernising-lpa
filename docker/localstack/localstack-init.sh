...
awslocal lambda create-function \
  --environment Variables="{AWS_BASE_URL=$AWS_BASE_URL,LPAS_TABLE=$LPAS_TABLE,GOVUK_NOTIFY_IS_PRODUCTION=$GOVUK_NOTIFY_IS_PRODUCTION,APP_PUBLIC_URL=$APP_PUBLIC_URL,GOVUK_NOTIFY_BASE_URL=$GOVUK_NOTIFY_BASE_URL,UPLOADS_S3_BUCKET_NAME=$UPLOAD_S3_BUCKET,UID_BASE_URL=$UID_BASE_URL,SEARCH_ENDPOINT=$SEARCH_ENDPOINT,SEARCH_INDEXING_DISABLED=$SEARCH_INDEXING_DISABLED,LPA_STORE_BASE_URL=$LPA_STORE_BASE_URL,LPA_STORE_SECRET_ARN=$LPA_STORE_SECRET_ARN,S3_UPLOADS_KMS_KEY_ALIAS=$S3_UPLOADS_KMS_KEY_ALIAS,CERTIFICATE_PROVIDER_START_URL=$CERTIFICATE_PROVIDER_START_URL,ATTORNEY_START_URL=$ATTORNEY_START_URL}" \
  --region eu-west-1 \
