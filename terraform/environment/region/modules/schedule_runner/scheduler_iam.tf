...
data "aws_iam_policy_document" "schedule_runner" {
  ...
  statement {
    sid    = "${local.policy_region_prefix}LambdaKMSKeyAccess"
    effect = "Allow"
  ...
  statement {
    sid    = "${local.policy_region_prefix}AllowSecretManagerKMSKeyDecryption"
    effect = "Allow"
  ...
  statement {
    sid    = "${local.policy_region_prefix}LPAStoreJWTKeyAccess"
    effect = "Allow"
