...
data "aws_dynamodb_table" "lpas_table" {
  name = "${data.aws_default_tags.current.tags.environment-name}-Lpas"
  provider = aws.region
}
...
data "aws_iam_policy_document" "task_role_access_policy" {
  ...
  statement {
    ...
    resources = [
      data.aws_dynamodb_table.lpas_table.arn,
      "${data.aws_dynamodb_table.lpas_table.arn}/index/*",
    ]
  }
  ...
  app = jsonencode(
    {
      ...
      environment = [
        ...
        },
        {
          ...
          value = data.aws_dynamodb_table.lpas_table.name
        },
      ]
    }
  )
