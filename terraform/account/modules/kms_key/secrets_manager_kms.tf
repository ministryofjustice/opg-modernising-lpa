resource "aws_kms_key" "main" {
  description             = "${data.aws_default_tags.current.application} ${var.encrypted_resource} encryption key"
...
resource "aws_kms_replica_key" "main" {
  description             = "${data.aws_default_tags.current.application} ${var.encrypted_resource} multi-region replica key"
  deletion_window_in_days = var.deletion_window_in_days
