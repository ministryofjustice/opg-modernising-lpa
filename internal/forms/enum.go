...
type unmarshalEnumType[T enumType] interface {
	UnmarshalText([]byte) error
	*T
}

// Enum is a validatable enum form field.
type Enum[T enumType, O any, U unmarshalEnumType[T]] struct {
	Field
	Value      T
	Options    O
	parseError error
	validators []validator[enumType]
}

func NewEnum[T enumType, O any, U unmarshalEnumType[T]](name, label string, values O) *Enum[T, O, U] {
	f := &Enum[T, O, U]{}
	f.Name = name
	f.Label = label
	f.Options = values
...
func (f *Enum[T, O, U]) WithError(replace Error) *Enum[T, O, U] {
	if l := len(f.validators); l > 0 {
		f.validators[l-1] = withError[enumType]{replace: replace, wrapped: f.validators[l-1]}
	}
...
func (v enumSelectedValidator) Validate(e enumType) Error {
	if *v.parseError != nil {
