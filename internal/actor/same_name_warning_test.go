...
func TestNewSameNameWarning(t *testing.T) {
	assert.Nil(t, NewSameNameWarning(TypeNone, TypeDonor, ""))
	assert.Nil(t, NewSameNameWarning(TypeDonor, TypeNone, ""))

	assert.Equal(t, &SameNameWarning{
		actor:    TypeDonor,
		matches:  TypeAttorney,
		fullName: "a",
	}, NewSameNameWarning(TypeDonor, TypeAttorney, "a"))
}
