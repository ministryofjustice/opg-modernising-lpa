...
func TestCertificateProviderStoreCreate(t *testing.T) {
	...
	uid := actoruid.New()
	now := time.Now()
	details := &actor.CertificateProviderProvidedDetails{PK: "LPA#123", SK: dynamo.CertificateProviderKey("456"), LpaID: "123", UpdatedAt: now, UID: uid, Email: "a@b.com"}
	...
	dynamoClient.EXPECT().
		...
		Return(nil)
	dynamoClient.EXPECT().
		Create(ctx, lpaLink{PK: "LPA#123", SK: dynamo.SubKey("456"), DonorKey: dynamo.DonorKey("session-id"), ActorType: actor.TypeCertificateProvider, UpdatedAt: now}).
		Return(nil)
...
func TestCertificateProviderStoreGetAny(t *testing.T) {
	...
	dynamoClient := newMockDynamoClient(t)
	dynamoClient.
		ExpectOneByPartialSK(ctx, "LPA#123", dynamo.CertificateProviderKey(""), &actor.CertificateProviderProvidedDetails{LpaID: "123"}, nil)
...
func TestCertificateProviderStoreGetAnyOnError(t *testing.T) {
	...
	dynamoClient := newMockDynamoClient(t)
	dynamoClient.
		ExpectOneByPartialSK(ctx, "LPA#123", dynamo.CertificateProviderKey(""), &actor.CertificateProviderProvidedDetails{LpaID: "123"}, expectedError)
...
func TestCertificateProviderStoreGet(t *testing.T) {
	...
	dynamoClient := newMockDynamoClient(t)
	dynamoClient.
		ExpectOne(ctx, "LPA#123", dynamo.CertificateProviderKey("456"), &actor.CertificateProviderProvidedDetails{LpaID: "123"}, nil)
...
func TestCertificateProviderStoreGetOnError(t *testing.T) {
	...
	dynamoClient := newMockDynamoClient(t)
	dynamoClient.
		ExpectOne(ctx, "LPA#123", dynamo.CertificateProviderKey("456"), &actor.CertificateProviderProvidedDetails{LpaID: "123"}, expectedError)
...
func TestCertificateProviderStorePut(t *testing.T) {
	...
	dynamoClient := newMockDynamoClient(t)
	dynamoClient.EXPECT().
		Put(ctx, &actor.CertificateProviderProvidedDetails{PK: "LPA#123", SK: dynamo.CertificateProviderKey("456"), LpaID: "123", UpdatedAt: now}).
		Return(nil)
	...
	err := certificateProviderStore.Put(ctx, &actor.CertificateProviderProvidedDetails{PK: "LPA#123", SK: dynamo.CertificateProviderKey("456"), LpaID: "123"})
	assert.Nil(t, err)
}
