...
func TestCertificateProviderStoreCreate(t *testing.T) {
	...
	certificateProvider, err := certificateProviderStore.Create(ctx, "session-id", uid, actor.Paper)
	assert.Nil(t, err)
...
func TestCertificateProviderStoreCreateWhenSessionMissing(t *testing.T) {
	...
	_, err := certificateProviderStore.Create(ctx, "session-id", actoruid.New(), actor.Online)
