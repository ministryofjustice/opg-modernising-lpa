...
func TestGetChangeMobileNumber(t *testing.T) {
	for actorType, firstNames := range map[actor.Type]string{
		...
		t.Run(actorType.String(), func(t *testing.T) {
			...
			err := ChangeMobileNumber(template.Execute, newMockWitnessCodeSender(t), actorType)(testAppData, w, r, &actor.Lpa{
...
func TestGetChangeMobileNumberWhenTemplateErrors(t *testing.T) {
	...
	err := ChangeMobileNumber(template.Execute, newMockWitnessCodeSender(t), actor.TypeCertificateProvider)(testAppData, w, r, &actor.Lpa{})
	resp := w.Result()
...
func TestPostChangeMobileNumber(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			err := ChangeMobileNumber(nil, witnessCodeSender, tc.actorType)(testAppData, w, r, &actor.Lpa{
				ID:                    "lpa-id",
				DonorIdentityUserData: identity.UserData{OK: true},
			})
			resp := w.Result()
...
func TestPostChangeMobileNumberWhenSendErrors(t *testing.T) {
	...
	err := ChangeMobileNumber(nil, witnessCodeSender, actor.TypeCertificateProvider)(testAppData, w, r, &actor.Lpa{ID: "lpa-id"})
	assert.Equal(t, expectedError, err)
}
...
func TestPostChangeMobileNumberWhenSendErrorsWithTooManyRequests(t *testing.T) {
	...
	err := ChangeMobileNumber(template.Execute, witnessCodeSender, actor.TypeCertificateProvider)(testAppData, w, r, &actor.Lpa{ID: "lpa-id"})
	resp := w.Result()
