...
func TestGetHowShouldReplacementAttorneysMakeDecisionsFromStore(t *testing.T) {
	...
	template.
		On("Execute", w, &howShouldReplacementAttorneysMakeDecisionsData{
			...
			Options: actor.AttorneysActValues,
			Lpa:     &page.Lpa{ReplacementAttorneyDecisions: actor.AttorneyDecisions{Details: "some decisions", How: actor.Jointly}},
		}).
		Return(nil)
...
func TestPostHowShouldReplacementAttorneysMakeDecisions(t *testing.T) {
	...
	donorStore.
		On("Put", r.Context(), &page.Lpa{ID: "lpa-id", ReplacementAttorneyDecisions: actor.AttorneyDecisions{How: actor.Jointly}}).
		Return(nil)

	template := newMockTemplate(t)

	err := HowShouldReplacementAttorneysMakeDecisions(template.Execute, donorStore)(testAppData, w, r, &page.Lpa{ID: "lpa-id"})
	resp := w.Result()
...
func TestPostHowShouldReplacementAttorneysMakeDecisionsFromStore(t *testing.T) {
	testCases := map[string]struct {
		...
		"existing details not set": {
			...
			attorneys: actor.Attorneys{Attorneys: []actor.Attorney{{FirstNames: "a", Email: "a"}}},
