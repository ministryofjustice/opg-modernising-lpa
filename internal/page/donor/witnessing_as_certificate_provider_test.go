...
func TestGetWitnessingAsCertificateProvider(t *testing.T) {
	...
	err := WitnessingAsCertificateProvider(template.Execute, nil, nil, time.Now, nil)(testAppData, w, r, &actor.Lpa{})
	resp := w.Result()
...
func TestPostWitnessingAsCertificateProvider(t *testing.T) {
	...
	for name, tc := range testcases {
		t.Run(name, func(t *testing.T) {
			...
			err := WitnessingAsCertificateProvider(nil, donorStore, shareCodeSender, func() time.Time { return now }, certificateProviderStore)(testAppData, w, r, &actor.Lpa{
				ID:                       "lpa-id",
				DonorIdentityUserData:    identity.UserData{OK: true},
				CertificateProviderCodes: actor.WitnessCodes{{Code: "1234", Created: now}},
				CertificateProvider:      actor.CertificateProvider{FirstNames: "Fred"},
...
func TestPostWitnessingAsCertificateProviderCodeTooOld(t *testing.T) {
	...
	donorStore := newMockDonorStore(t)
	donorStore.
		On("Put", r.Context(), mock.MatchedBy(func(lpa *actor.Lpa) bool {
			lpa.WitnessCodeLimiter = nil
			return assert.Equal(t, lpa, &actor.Lpa{
	...
	err := WitnessingAsCertificateProvider(template.Execute, donorStore, nil, time.Now, nil)(testAppData, w, r, &actor.Lpa{
		CertificateProviderCodes: actor.WitnessCodes{{Code: "1234", Created: invalidCreated}},
	})
	resp := w.Result()
