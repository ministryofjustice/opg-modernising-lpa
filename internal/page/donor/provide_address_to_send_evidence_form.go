...
func ProvideAddressToSendEvidenceForm(logger Logger, tmpl template.Template, addressClient AddressClient, donorStore DonorStore) Handler {
	return func(appData page.AppData, w http.ResponseWriter, r *http.Request, lpa *page.Lpa) error {
		...
		if lpa.EvidenceFormAddress.Line1 != "" {
			data.Form.Action = "manual"
			data.Form.Address = &lpa.EvidenceFormAddress
		}
		...
		if r.Method == http.MethodPost {
			...
			setAddress := func(address place.Address) error {
				lpa.EvidenceFormAddress = *data.Form.Address
			...
			case "manual":
				if data.Errors.None() {
					...
					return appData.Redirect(w, r, lpa, page.Paths.HowToSendEvidence.Format(lpa.ID))
				}
