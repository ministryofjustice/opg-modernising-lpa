...
func TestGetYourIndependentWitnessMobile(t *testing.T) {
	...
	err := YourIndependentWitnessMobile(template.Execute, nil)(testAppData, w, r, &actor.Lpa{})
	resp := w.Result()
...
func TestPostYourIndependentWitnessMobile(t *testing.T) {
	testCases := map[string]struct {
		form                         url.Values
		yourIndependentWitnessMobile actor.IndependentWitness
	}{
		"valid": {
			form: url.Values{
				"mobile": {"07535111111"},
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			err := YourIndependentWitnessMobile(nil, donorStore)(testAppData, w, r, &actor.Lpa{ID: "lpa-id"})
			resp := w.Result()
...
func TestPostYourIndependentWitnessMobileWhenStoreErrors(t *testing.T) {
	...
	err := YourIndependentWitnessMobile(nil, donorStore)(testAppData, w, r, &actor.Lpa{})
