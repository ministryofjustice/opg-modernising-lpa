		...
		if organisation == "1" {
			...
			if lpaCount, err := strconv.Atoi(lpa); err == nil {
				...
				for range lpaCount {
					...
					if setLPAProgress {
						donor, fns, err = updateLPAProgress(donorFixtureData, donor, random.String(16), r, certificateProviderStore, attorneyStore, documentStore, eventClient, shareCodeStore, true)
						if err != nil {
							return err
						}
					}
