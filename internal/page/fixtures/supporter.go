	...
	return func(appData page.AppData, w http.ResponseWriter, r *http.Request) error {
		...
		if organisation == "1" {
			...
			if accessCode != "" {
				...
				if linkDonor {
					shareCodeData.PK = dynamo.DonorShareKey(accessCode)
					shareCodeData.SK = dynamo.DonorInviteKey(shareCodeData.SessionID, shareCodeData.LpaID)
			...
			if invitedMembers != "" {
				...
				for i, member := range invitedOrgMemberNames {
					...
					invite := &actor.MemberInvite{
						...
						SK:               dynamo.MemberInviteKey(base64.StdEncoding.EncodeToString([]byte(email))),
						CreatedAt:        now,
