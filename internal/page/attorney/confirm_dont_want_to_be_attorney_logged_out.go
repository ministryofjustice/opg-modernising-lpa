...
func ConfirmDontWantToBeAttorneyLoggedOut(tmpl template.Template, shareCodeStore ShareCodeStore, lpaStoreResolvingService LpaStoreResolvingService, lpaStoreClient LpaStoreClient, sessionStore SessionStore, notifyClient NotifyClient, appPublicURL string) page.Handler {
	return func(appData page.AppData, w http.ResponseWriter, r *http.Request) error {
		...
		if r.Method == http.MethodPost {
			...
			email := notify.AttorneyOptedOutEmail{
				AttorneyFullName:  findAttorneyFullName(lpa, shareCode.ActorUID, shareCode.IsTrustCorporation, shareCode.IsReplacementAttorney),
			...
			if !lpa.CannotRegister {
				if err := lpaStoreClient.SendAttorneyOptOut(ctx, lpa.LpaUID, actoruid.Service); err != nil {
					return err
				}
			}
...
func findAttorneyFullName(lpa *lpastore.Lpa, uid actoruid.UID, isTrustCorporation, isReplacement bool) string {
