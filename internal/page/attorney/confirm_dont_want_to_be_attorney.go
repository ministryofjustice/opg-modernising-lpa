...
func ConfirmDontWantToBeAttorney(tmpl template.Template, lpaStoreResolvingService LpaStoreResolvingService, lpaStoreClient LpaStoreClient, attorneyStore AttorneyStore, notifyClient NotifyClient, appPublicURL string) Handler {
	return func(appData page.AppData, w http.ResponseWriter, r *http.Request, attorneyProvidedDetails *actor.AttorneyProvidedDetails) error {
		...
		if r.Method == http.MethodPost {
			email := notify.AttorneyOptedOutEmail{
				AttorneyFullName:  findAttorneyFullName(lpa, attorneyProvidedDetails.UID, attorneyProvidedDetails.IsTrustCorporation, attorneyProvidedDetails.IsReplacement),
