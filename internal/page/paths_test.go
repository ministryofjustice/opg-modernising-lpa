...
func TestLpaPathRedirect(t *testing.T) {
	testCases := map[string]struct {
		...
		"allowed from": {
			...
			lpa:      &actor.Lpa{ID: "lpa-id", Tasks: actor.DonorTasks{YourDetails: actor.TaskCompleted, ChooseAttorneys: actor.TaskCompleted}},
...
func TestLpaPathRedirectQuery(t *testing.T) {
	...
	err := Paths.TaskList.RedirectQuery(w, r, AppData{Lang: localize.En}, &actor.Lpa{ID: "lpa-id"}, url.Values{"q": {"1"}})
	resp := w.Result()
