...
// OneActive provides a mock function with given fields: ctx, pk, sk, now, v
func (_m *mockDynamoClient) OneActive(ctx context.Context, pk dynamo.PK, sk dynamo.ExpiresAtKeyType, now time.Time, v interface{}) error {
	ret := _m.Called(ctx, pk, sk, now, v)

	if len(ret) == 0 {
		panic("no return value specified for OneActive")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, dynamo.PK, dynamo.ExpiresAtKeyType, time.Time, interface{}) error); ok {
		r0 = rf(ctx, pk, sk, now, v)
	} else {
		r0 = ret.Error(0)
	}
...
// mockDynamoClient_OneActive_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'OneActive'
type mockDynamoClient_OneActive_Call struct {
	*mock.Call
}

// OneActive is a helper method to define mock.On call
//   - ctx context.Context
//   - pk dynamo.PK
//   - sk dynamo.ExpiresAtKeyType
//   - now time.Time
//   - v interface{}
func (_e *mockDynamoClient_Expecter) OneActive(ctx interface{}, pk interface{}, sk interface{}, now interface{}, v interface{}) *mockDynamoClient_OneActive_Call {
	return &mockDynamoClient_OneActive_Call{Call: _e.mock.On("OneActive", ctx, pk, sk, now, v)}
}

func (_c *mockDynamoClient_OneActive_Call) Run(run func(ctx context.Context, pk dynamo.PK, sk dynamo.ExpiresAtKeyType, now time.Time, v interface{})) *mockDynamoClient_OneActive_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(dynamo.PK), args[2].(dynamo.ExpiresAtKeyType), args[3].(time.Time), args[4].(interface{}))
	})
	return _c
}

func (_c *mockDynamoClient_OneActive_Call) Return(_a0 error) *mockDynamoClient_OneActive_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *mockDynamoClient_OneActive_Call) RunAndReturn(run func(context.Context, dynamo.PK, dynamo.ExpiresAtKeyType, time.Time, interface{}) error) *mockDynamoClient_OneActive_Call {
	_c.Call.Return(run)
	return _c
}
