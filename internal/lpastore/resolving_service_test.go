...
func TestResolvingServiceGet(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"paper": {
			...
			resolved: &lpadata.Lpa{LpaID: "1", Submitted: true},
			expected: &lpadata.Lpa{
				...
				CertificateProvider: lpadata.CertificateProvider{Relationship: lpadata.Personally},
			},
		},
		"voucher not allowed": {
			donor: &donordata.Provided{
...
func TestResolvingServiceResolve(t *testing.T) {
	testcases := map[string]struct {
		...
		"paper": {
			...
			resolved: &lpadata.Lpa{LpaID: "1", LpaUID: "M-1111", Submitted: true},
			expected: &lpadata.Lpa{
				LpaOwnerKey:         dynamo.LpaOwnerKey(dynamo.DonorKey("PAPER")),
				LpaID:               "1",
				LpaUID:              "M-1111",
