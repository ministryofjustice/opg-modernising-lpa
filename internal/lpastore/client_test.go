...
func TestClientServiceContract(t *testing.T) {
	...
	pact.
		...
		WillRespondWith(dsl.Response{
			...
			Body:    `{"errors":"LPA with UID already exists"}`,
		})
	...
	pact.Verify(func() error {
		...
		assert.Equal(t, responseError{name: "expected 201 response but got 400", body: `{"errors":"LPA with UID already exists"}`}, err)
		return nil
	})
}
