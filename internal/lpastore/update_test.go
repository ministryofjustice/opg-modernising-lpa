...
func TestClientSendPaperCertificateProviderAccessOnline(t *testing.T) {
	json := `{"type":"PAPER_CERTIFICATE_PROVIDER_ACCESS_ONLINE","changes":[{"key":"/certificateProvider/email","old":null,"new":"a@example.com"}]}`
	uid, _ := actoruid.Parse("dc487ebb-b39d-45ed-bb6a-7f950fd355c9")

	lpa := lpadata.Lpa{LpaUID: "lpa-uid", CertificateProvider: lpadata.CertificateProvider{UID: uid}}
	...
	client := New("http://base", secretsClient, "secret", doer)
	client.now = func() time.Time { return time.Date(2000, time.January, 2, 3, 4, 5, 6, time.UTC) }
	err := client.SendPaperCertificateProviderAccessOnline(ctx, &lpa, "a@example.com")

	assert.Nil(t, err)
}

func TestClientSendPaperAttorneyAccessOnline(t *testing.T) {
	json := `{"type":"PAPER_ATTORNEY_ACCESS_ONLINE","changes":[{"key":"/attorneys/dc487ebb-b39d-45ed-bb6a-7f950fd355c9/email","old":null,"new":"a@example.com"}]}`
	uid, _ := actoruid.Parse("dc487ebb-b39d-45ed-bb6a-7f950fd355c9")
	...
	client := New("http://base", secretsClient, "secret", doer)
	client.now = func() time.Time { return time.Date(2000, time.January, 2, 3, 4, 5, 6, time.UTC) }
	err := client.SendPaperAttorneyAccessOnline(ctx, "lpa-uid", "a@example.com", uid)
