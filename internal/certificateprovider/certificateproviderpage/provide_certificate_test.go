...
func TestPostProvideCertificateWhenWrongLanguageUsed(t *testing.T) {
	form := url.Values{
		"agree-to-statement": {"1"},
	...
	template.EXPECT().
		Execute(w, mock.MatchedBy(func(data *provideCertificateData) bool {
			return assert.Equal(t, []forms.Field{data.Form.AgreeToStatement.Field}, data.Form.Errors) &&
				assert.Equal(t, "toSignCertificateYouMustViewInLanguage:Lang=cy", data.Form.AgreeToStatement.Error.Format(testAppData.Localizer))
		})).
		Return(nil)

	err := ProvideCertificate(template.Execute, nil, nil, nil, nil, nil, nil, testNowFn, "donorStartURL", nil)(testAppData, w, r, &certificateproviderdata.Provided{}, &lpadata.Lpa{SignedAt: testNow, Language: localize.Cy, WitnessedByCertificateProviderAt: testNow})
	resp := w.Result()
