...
func ProvideCertificate(
	...
	return func(appData appcontext.Data, w http.ResponseWriter, r *http.Request, certificateProvider *certificateproviderdata.Provided, lpa *lpadata.Lpa) error {
		...
		data.Form.AgreeToStatement.Set(!certificateProvider.SignedAt.IsZero())
...
type provideCertificateForm struct {
	forms.Form
	Submittable      *forms.String
	AgreeToStatement *forms.Bool
...
func newProvideCertificateForm(l Localizer, lang localize.Lang, certificateProviderFullName string) *provideCertificateForm {
	return &provideCertificateForm{
		Submittable:      forms.NewString("submittable", ""),
		AgreeToStatement: forms.NewBool("agree-to-statement", l.Format("iAgreeToTheseStatements", map[string]any{"FullName": certificateProviderFullName})),
		lpaLanguage:      lang,
	}
}

func (f *provideCertificateForm) Parse(r *http.Request) bool {
	ok := f.ParsePostForm(r, f.Submittable, f.AgreeToStatement)

	if f.Submittable.Value != "cannot-submit" && !f.AgreeToStatement.Value {
		f.AgreeToStatement.Error = toSignAsCertificateProviderError{}
...
type toSignAsCertificateProviderError struct{}

func (e toSignAsCertificateProviderError) Format(l forms.Localizer) string {
	return l.Format("errorSelect", map[string]any{
		"Label": l.T("toSignAsCertificateProvider"),
	})
}
...
func (e toSignCertificateYouMustViewInLanguageError) Format(l forms.Localizer) string {
