...
// toConfirmLpaStubHash filters the fields used for hashing to only those
// that make up LPA stub data
type toConfirmLpaStubHash Provided

func (c toConfirmLpaStubHash) HashInclude(field string, _ any) (bool, error) {
	if c.LpaStubHashVersion > currentLpaStubHashVersion {
		return false, errors.New("LpaStubHashVersion too high")
	}

	return field == "Donor" || field == "LpaType", nil
}
...
func (p *Provided) generateLpaStubHash() (uint64, error) {
	return hashstructure.Hash(toConfirmLpaStubHash(*p), &hashstructure.HashOptions{TagName: "stubhash"})
}
...
func (p *Provided) CertificateProviderSharesLastName() bool {
	...
	if names.EqualHyphenated(p.Donor.LastName, p.CertificateProvider.LastName) {
		return true
	}

	for _, attorney := range append(p.Attorneys.Attorneys, p.ReplacementAttorneys.Attorneys...) {
		if names.EqualHyphenated(attorney.LastName, p.CertificateProvider.LastName) {
			return true
		}
	}
