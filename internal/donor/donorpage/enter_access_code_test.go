...
func TestPostEnterAccessCode(t *testing.T) {
	...
	donorStore.EXPECT().
		Link(newCtx, accessCode, "a@example.com").
		Return(nil)

	logger := newMockLogger(t)
	logger.EXPECT().
		InfoContext(newCtx, "donor access added", slog.String("lpa_id", "lpa-id"))

	eventClient := newMockEventClient(t)
	eventClient.EXPECT().
		SendMetric(newCtx, event.CategoryFunnelStartRate, event.MeasureOnlineDonor).
		Return(nil)
	...
	template.EXPECT().
		Execute(mock.Anything, mock.MatchedBy(func(data enterAccessCodeData) bool {
			return assert.Equal(t, validation.With(form.FieldNames.AccessCode, validation.IncorrectError{Label: "accessCode"}).
				With(form.FieldNames.DonorLastName, validation.IncorrectError{Label: "donorLastName"}),
				data.Errors)
		})).
		Return(nil)
