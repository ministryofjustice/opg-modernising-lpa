...
func TestGetWhenCanTheLpaBeUsedFromStore(t *testing.T) {
	...
	template.EXPECT().
		Execute(w, &whenCanTheLpaBeUsedData{
			App:   testAppData,
			Donor: &donordata.Provided{WhenCanTheLpaBeUsed: donordata.CanBeUsedWhenHasCapacity},
			Form: &whenCanTheLpaBeUsedForm{
	...
	err := WhenCanTheLpaBeUsed(template.Execute, nil)(testAppData, w, r, &donordata.Provided{WhenCanTheLpaBeUsed: donordata.CanBeUsedWhenHasCapacity})
	resp := w.Result()
...
func TestPostWhenCanTheLpaBeUsed(t *testing.T) {
	...
	donorStore.EXPECT().
		Put(r.Context(), &donordata.Provided{
			...
			Tasks:               donordata.Tasks{YourDetails: actor.TaskCompleted, ChooseAttorneys: actor.TaskCompleted, WhenCanTheLpaBeUsed: actor.TaskCompleted},
		}).
		Return(nil)
...
func TestPostWhenCanTheLpaBeUsedWhenStoreErrors(t *testing.T) {
	...
	donorStore := newMockDonorStore(t)
	donorStore.EXPECT().
		Put(r.Context(), &donordata.Provided{WhenCanTheLpaBeUsed: donordata.CanBeUsedWhenHasCapacity, Tasks: donordata.Tasks{WhenCanTheLpaBeUsed: actor.TaskCompleted}}).
		Return(expectedError)
