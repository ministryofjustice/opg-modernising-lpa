...
func TestGetWarningInterruption(t *testing.T) {
	query := url.Values{
	...
	template.EXPECT().
		Execute(w, WarningInterruptionData{
			App:           appData,
			Donor:         provided,
			Attorney:      donordata.Attorney{UID: testUID, FirstNames: "Jane", LastName: "Doe"},
			Notifications: []page.Notification{{Heading: "pleaseReviewTheInformationYouHaveEntered", BodyHTML: "translatedWarning"}},
