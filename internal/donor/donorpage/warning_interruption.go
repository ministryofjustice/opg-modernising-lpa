...
func WarningInterruption(tmpl template.Template) Handler {
	return func(appData appcontext.Data, w http.ResponseWriter, r *http.Request, provided *donordata.Provided) error {
		...
		switch data.From {
		case donor.PathChooseAttorneys.Format(appData.LpaID), donor.PathChooseReplacementAttorneys.Format(appData.LpaID):
			...
			if data.From == donor.PathChooseReplacementAttorneys.Format(appData.LpaID) {
		...
		case donor.PathCertificateProviderDetails.Format(appData.LpaID):
			...
			nameWarning := actor.NewSameNameWarning(
				...
				matches,
				provided.CertificateProvider.FirstNames,
				provided.CertificateProvider.LastName,
			)
		...
		case donor.PathEnterCorrespondentDetails.Format(appData.LpaID):
			...
			if correspondentNameMatchesDonor(provided, provided.Correspondent.FirstNames, provided.Correspondent.LastName) {
				data.Notifications = append(data.Notifications, page.Notification{
					...
					BodyHTML: actor.NewSameNameWarning(
						...
						actor.TypeDonor,
						provided.Correspondent.FirstNames,
						provided.Correspondent.LastName,
					).Format(appData.Localizer),
				})
			}
		case donor.PathChoosePeopleToNotify.Format(appData.LpaID):
			uid, err := actoruid.Parse(r.FormValue("id"))
			if err != nil {
				return donor.PathTaskList.RedirectQuery(w, r, appData, provided, nil)
			}
		...
		case donor.PathYourAuthorisedSignatory.Format(appData.LpaID):
			...
			if !matches.IsNone() {
				data.Notifications = append(data.Notifications, page.Notification{
					...
					BodyHTML: actor.NewSameNameWarning(
						...
						matches,
						provided.AuthorisedSignatory.FirstNames,
						provided.AuthorisedSignatory.LastName,
					).Format(appData.Localizer),
				})
			}

		case donor.PathYourIndependentWitness.Format(appData.LpaID):
			...
			if !matches.IsNone() {
				data.Notifications = append(data.Notifications, page.Notification{
					...
					BodyHTML: actor.NewSameNameWarning(
						...
						matches,
						provided.IndependentWitness.FirstNames,
						provided.IndependentWitness.LastName,
					).Format(appData.Localizer),
				})
			}
		}
