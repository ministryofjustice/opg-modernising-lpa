	...
	return func(appData appcontext.Data, w http.ResponseWriter, r *http.Request, provided *donordata.Provided) error {
		...
		if r.Method == http.MethodPost {
			...
			if data.Errors.None() {
				...
				redirectPage := donor.PathYourNonUKAddress
				if data.Form.CountryCode == "GB" {
					redirectPage = donor.PathYourAddress
				}

				redirectQuery := url.Values{}
				if r.FormValue("makingAnotherLPA") == "1" {
					redirectQuery.Add("makingAnotherLPA", "1")
				}
				if r.FormValue("next") != "" {
					redirectQuery.Add("from", r.FormValue("next"))
				}

				return redirectPage.RedirectQuery(w, r, appData, provided, redirectQuery)
			}
		}
