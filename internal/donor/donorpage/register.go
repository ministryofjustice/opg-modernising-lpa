...
func Register(
	...
	handleWithDonor(donor.PathDeleteThisLpa, page.None,
		DeleteLpa(logger, tmpls.Get("delete_this_lpa.gohtml"), donorStore, notifyClient, certificateProviderStartURL, eventClient))
	...
	handleWithDonor(donor.PathContactDetails, page.None,
		Guidance(tmpls.Get("contact_details.gohtml")))
	handleWithDonor(donor.PathContactEmail, page.CanGoBack,
		YourEmail(tmpls.Get("contact_email.gohtml"), donorStore, nil))
	...
	{
		service := donor.NewCertificateProviderService(donorStore, reuseStore, scheduledStore, accessCodeStore, certificateProviderStore)
		...
		handleWithDonor(donor.PathChooseNewCertificateProvider, page.None,
			RemoveCertificateProvider(tmpls.Get("choose_new_certificate_provider.gohtml"), service, notifyClient, appPublicURL))
		...
		handleWithDonor(donor.PathHowWouldCertificateProviderPreferToCarryOutTheirRole, page.CanGoBack,
			HowWouldCertificateProviderPreferToCarryOutTheirRole(tmpls.Get("how_would_certificate_provider_prefer_to_carry_out_their_role.gohtml"), donorStore, certificateProviderStore, reuseStore, accessCodeStore, accessCodeSender, time.Now))
