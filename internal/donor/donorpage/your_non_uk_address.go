...
func YourNonUKAddress(tmpl template.Template, donorStore DonorStore) Handler {
	return func(appData appcontext.Data, w http.ResponseWriter, r *http.Request, provided *donordata.Provided) error {
		...
		query := url.Values{}
		if r.FormValue("from") != "" {
			query.Add("from", r.FormValue("from"))
		}
		if data.MakingAnotherLPA {
			query.Add("makingAnotherLPA", "1")
		}
		data.WhatCountryLink = donor.PathWhatCountryDoYouLiveIn.FormatQuery(provided.LpaID, query)

		if r.Method == http.MethodPost {
			data.Form = readYourNonUKAddressForm(r)
			data.Errors = data.Form.Validate()

			if r.PostForm.Has("live-in-uk") {
				...
				return donor.PathYourAddress.RedirectQuery(w, r, appData, provided, query)
			}
