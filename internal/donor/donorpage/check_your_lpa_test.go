...
func TestPostCheckYourLpaWhenNotChanged(t *testing.T) {
	...
	donor := &donordata.Provided{
		LpaID:               "lpa-id",
		CheckedAt:           testNow,
		Tasks:               task.DonorTasks{CheckYourLpa: task.StateCompleted},
	...
	for _, existingTaskState := range testCases {
		t.Run(existingTaskState.String(), func(t *testing.T) {
			...
			provided := &donordata.Provided{
				LpaID:               "lpa-id",
				Hash:                5,
				Tasks:               task.DonorTasks{CheckYourLpa: existingTaskState},
...
func TestPostCheckYourLpaWhenShareCodeSenderErrors(t *testing.T) {
	...
	donor := &donordata.Provided{
		LpaID: "lpa-id",
		Hash:  5,
		Tasks: task.DonorTasks{CheckYourLpa: task.StateInProgress},
	}
