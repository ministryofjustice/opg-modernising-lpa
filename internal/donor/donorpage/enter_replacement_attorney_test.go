...
func TestGetEnterReplacementAttorney(t *testing.T) {
	...
	for name, tc := range testcases {
		t.Run(name, func(t *testing.T) {
			...
			err := EnterReplacementAttorney(template.Execute, nil)(testAppData, w, r, &donordata.Provided{
				Type:      tc.lpaType,
				Attorneys: tc.attorneys,
			})
			resp := w.Result()
...
func TestGetEnterReplacementAttorneyWhenNoID(t *testing.T) {
	...
	err := EnterReplacementAttorney(nil, nil)(testAppData, w, r, &donordata.Provided{LpaID: "lpa-id", ReplacementAttorneys: donordata.Attorneys{Attorneys: []donordata.Attorney{{FirstNames: "John", UID: actoruid.New()}}}})
	resp := w.Result()
...
func TestGetEnterReplacementAttorneyDobWarningIsAlwaysShown(t *testing.T) {
	w := httptest.NewRecorder()
	r, _ := http.NewRequest(http.MethodGet, "/?id="+testUID.String(), nil)
...
func TestPostEnterReplacementAttorneyAttorneyDoesNotExists(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			err := EnterReplacementAttorney(nil, donorStore)(testAppData, w, r, &donordata.Provided{LpaID: "lpa-id", Donor: donordata.Donor{FirstNames: "Jane", LastName: "Doe"}})
			resp := w.Result()
...
func TestPostEnterReplacementAttorneyAttorneyExists(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			err := EnterReplacementAttorney(nil, donorStore)(testAppData, w, r, &donordata.Provided{
				LpaID: "lpa-id",
				Donor: donordata.Donor{FirstNames: "Jane", LastName: "Doe"},
				ReplacementAttorneys: donordata.Attorneys{Attorneys: []donordata.Attorney{
					{
...
func TestPostEnterReplacementAttorneyNameWarningOnlyShownWhenAttorneyAndFormNamesAreDifferent(t *testing.T) {
	form := url.Values{
		"first-names":         {"Jane"},
		"last-name":           {"Doe"},
		"date-of-birth-day":   {"2"},
...
func TestPostEnterReplacementAttorneyWhenInputRequired(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			err := EnterReplacementAttorney(template.Execute, nil)(testAppData, w, r, &donordata.Provided{Donor: donordata.Donor{FirstNames: "Jane", LastName: "Doe"}})
			resp := w.Result()
...
func TestPostEnterReplacementAttorneyWhenStoreErrors(t *testing.T) {
	form := url.Values{
		"first-names":         {"John"},
		"last-name":           {"Doe"},
		"email":               {"john@example.com"},
