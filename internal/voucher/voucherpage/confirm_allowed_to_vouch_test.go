...
func TestPostConfirmAllowedToVouchWhenNo(t *testing.T) {
	...
	err := ConfirmAllowedToVouch(nil, lpaStoreResolvingService, nil, vouchFailer.Execute)(testAppData, w, r, provided)
	resp := w.Result()
...
func TestPostConfirmAllowedToVouchWhenNoWhenFailVouchError(t *testing.T) {
	...
	err := ConfirmAllowedToVouch(nil, lpaStoreResolvingService, nil, vouchFailer.Execute)(testAppData, w, r, &voucherdata.Provided{LpaID: "lpa-id"})
	resp := w.Result()
