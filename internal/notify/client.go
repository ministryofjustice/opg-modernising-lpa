...
func (c *Client) SendActorEmail(ctx context.Context, to, lpaUID string, email Email) error {
	exists, err := c.recentlyCreated(ctx, c.makeReference(lpaUID, to, email))
	if err != nil {
		return err
	}
	if exists {
...
func (c *Client) recentlyCreated(ctx context.Context, ref string) (bool, error) {
	...
	for _, notification := range resp.Notifications {
		if notification.CreatedAt.After(c.now().Add(-allowResendAfter)) {
			return true, nil
		}
	}
