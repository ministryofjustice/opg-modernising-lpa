...
func (s *Store) Pop(ctx context.Context, day time.Time) (Row, error) {
	var row Row
	if err := s.dynamoClient.OneByPK(ctx, dynamo.CronDayKey(day), &row); err != nil {
		return row, err
	}
...
func (s *Store) Put(ctx context.Context, row Row) error {
	row.PK = dynamo.CronDayKey(row.At)
	row.SK = dynamo.CronKey(row.At, int(row.Action))
	row.CreatedAt = s.now()
