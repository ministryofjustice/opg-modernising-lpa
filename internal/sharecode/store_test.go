...
func TestShareCodeStoreGet(t *testing.T) {
	...
	for name, tc := range testcases {
		t.Run(name, func(t *testing.T) {
			...
			data := sharecodedata.Link{LpaKey: "lpa-id", CreatedAt: testNow.AddDate(-2, 0, 1)}
...
func TestShareCodeStoreGetWhenLinked(t *testing.T) {
	...
	dynamoClient.EXPECT().
		...
		Return(nil).
		SetData(sharecodedata.Link{LpaLinkedAt: time.Now(), CreatedAt: testNow})
...
func TestShareCodeStoreGetDonor(t *testing.T) {
	...
	data := sharecodedata.Link{LpaKey: dynamo.LpaKey("lpa-id"), CreatedAt: testNow}
