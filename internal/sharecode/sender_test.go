...
func TestSendVoucherInvite(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		donor              donordata.Donor
		voucherCodeChannel lpadata.Channel
		...
		"sms": {
			...
			voucherCodeChannel: lpadata.ChannelPaper,
		...
		"email": {
			...
			voucherCodeChannel: lpadata.ChannelOnline,
	...
	for name, tc := range testcases {
		t.Run(name, func(t *testing.T) {
			...
			assert.Equal(t, tc.voucherCodeChannel, provided.VoucherCodeChannel)
