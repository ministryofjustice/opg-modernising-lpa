...
const indexName = "lpas4"

type Client struct {
	svc *opensearchapi.Client
}
...
func NewClient(cfg aws.Config, endpoint string) (*Client, error) {
	...
	if _, err := svc.Info(context.Background(), nil); err != nil {
		return nil, fmt.Errorf("search could not connect: %w", err)
	}
...
func (c *Client) CreateIndices(ctx context.Context) error {
	_, err := c.svc.Indices.Exists(ctx, opensearchapi.IndicesExistsReq{Indices: []string{indexName}})
	if err == nil {
		return nil
	}
	...
	if _, err := c.svc.Indices.Create(ctx, req); err != nil {
