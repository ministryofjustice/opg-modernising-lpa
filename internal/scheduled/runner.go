...
type ActionFunc func(ctx context.Context, row Row) error
...
func (r *Runner) Run(ctx context.Context) error {
	...
	for {
		...
		select {
		case <-ctx.Done():
			return nil // TODO: should this be ctx.Err()?
		case <-ticker:
			continue
		}
	}
...
func (r *Runner) stepCancelDonorIdentity(ctx context.Context, row Row) error {
	...
	if !ok {
		return fmt.Errorf("incorrect TargetPK: %s", row.TargetPK.Unwrap())
	}
