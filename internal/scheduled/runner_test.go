...
func TestRunnerRunWhenPeriodElapses(t *testing.T) {
	...
	row := Row{
		Action:   99,
		TargetPK: dynamo.WrapPK(dynamo.LpaKey("an-lpa")),
		TargetSK: dynamo.WrapSK(dynamo.DonorKey("a-donor")),
	}
...
func TestRunnerStepCancelDonorIdentity(t *testing.T) {
	...
	donorKey := dynamo.DonorKey("a-donor")
	row := Row{
		TargetPK: dynamo.WrapPK(lpaKey),
		TargetSK: dynamo.WrapSK(donorKey),
	}
...
func TestRunnerStepCancelDonorIdentityWhenTargetPKWrongType(t *testing.T) {
	row := Row{
		TargetPK: dynamo.WrapPK(dynamo.OrganisationKey("an-org")),
	}

	runner := &Runner{}
	err := runner.stepCancelDonorIdentity(ctx, row)

	assert.ErrorContains(t, err, "incorrect TargetPK: ORGANISATION#an-org")
}
