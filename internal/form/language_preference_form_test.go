...
func TestReadLanguagePreferenceForm(t *testing.T) {
	form := url.Values{FieldNames.LanguagePreference.LanguagePreference: {localize.En.String()}}
...
func TestLanguagePreferenceFormValidate(t *testing.T) {
	testcases := map[string]struct {
		...
		"invalid": {
			...
			errors: validation.With(FieldNames.LanguagePreference.LanguagePreference, validation.SelectError{Label: "a-label"}),
		},
	}
