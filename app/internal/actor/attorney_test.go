...
func TestAttorneysFullNames(t *testing.T) {
	testcases := map[string]struct {
		...
		"attorneys": {
			...
			expected: []string{"Bob Alan George Jones", "Samantha Smith", "Abby Helen Burns-Simpson"},
		},
		"trust corporation": {
			attorneys: Attorneys{
				TrustCorporation: TrustCorporation{Name: "Corp corp"},
			},
			expected: []string{"Corp corp"},
		},
		"both": {
			attorneys: Attorneys{
				TrustCorporation: TrustCorporation{Name: "Corp corp"},
			...
			expected: []string{"Corp corp", "Bob Jones"},
		},
	}

	for name, tc := range testcases {
		t.Run(name, func(t *testing.T) {
			assert.Equal(t, tc.expected, tc.attorneys.FullNames())
		})
	}
}
