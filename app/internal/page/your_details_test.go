...
func TestPostYourDetails(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			sessionStore.
				On("Get", r, "session").
				Return(&sessions.Session{Values: map[any]any{"donor": &DonorLoginSession{Email: "name@example.com"}}}, nil)
...
func TestPostYourDetailsWhenSessionProblem(t *testing.T) {
	testCases := map[string]struct {
		...
		"store error": {
			session: &sessions.Session{Values: map[any]any{"donor": &DonorLoginSession{Email: "name@example.com"}}},
			error:   expectedError,
		},
		"missing email": {
			session: &sessions.Session{},
