	...
	return func(appData page.AppData, w http.ResponseWriter, r *http.Request) error {
		...
		if r.Method == http.MethodPost {
			...
			if data.Errors.None() {
				...
				if len(lpa.Attorneys) > 1 && lpa.HowAttorneysMakeDecisions.How == actor.JointlyAndSeverally && lpa.HowShouldReplacementAttorneysStepIn == page.AllCanNoLongerAct && len(lpa.ReplacementAttorneys) > 1 {
					return appData.Redirect(w, r, lpa, appData.Paths.HowShouldReplacementAttorneysMakeDecisions)
				} else if lpa.HowReplacementAttorneysMakeDecisions.RequiresHappiness(len(lpa.ReplacementAttorneys)) {
