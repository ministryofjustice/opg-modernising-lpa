...
// Send provides a mock function with given fields: ctx, template, appData, email, identity, lpa
func (_m *mockShareCodeSender) Send(ctx context.Context, template notify.TemplateId, appData page.AppData, email string, identity bool, lpa *page.Lpa) error {
	ret := _m.Called(ctx, template, appData, email, identity, lpa)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, notify.TemplateId, page.AppData, string, bool, *page.Lpa) error); ok {
		r0 = rf(ctx, template, appData, email, identity, lpa)
	} else {
		r0 = ret.Error(0)
	}
