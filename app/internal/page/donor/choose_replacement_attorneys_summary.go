	...
	return func(appData page.AppData, w http.ResponseWriter, r *http.Request) error {
		...
		if r.Method == http.MethodPost {
			...
			if data.Errors.None() {
				...
				if len(lpa.ReplacementAttorneys) > 1 && len(lpa.Attorneys) > 1 && lpa.HowAttorneysMakeDecisions.How == actor.Jointly {
					redirectUrl = appData.Paths.HowShouldReplacementAttorneysMakeDecisions
				} else if len(lpa.Attorneys) > 1 && lpa.HowAttorneysMakeDecisions.How == actor.JointlyAndSeverally {
