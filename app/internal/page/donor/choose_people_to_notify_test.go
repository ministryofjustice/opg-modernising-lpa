...
func TestGetChoosePeopleToNotify(t *testing.T) {
	...
	err := ChoosePeopleToNotify(template.Func, lpaStore, page.MockRandom)(page.TestAppData, w, r)
	resp := w.Result()
...
func TestGetChoosePeopleToNotifyWhenStoreErrors(t *testing.T) {
	...
	err := ChoosePeopleToNotify(nil, lpaStore, page.MockRandom)(page.TestAppData, w, r)
	resp := w.Result()
...
func TestPostChoosePeopleToNotifyFromAnotherPage(t *testing.T) {
	testcases := map[string]struct {
		...
		"without from value": {
			"/?from=&id=123",
			"/lpa/lpa-id" + page.Paths.ChoosePeopleToNotifyAddress + "?id=123",
		},
		"missing from key": {
			"/?id=123",
			"/lpa/lpa-id" + page.Paths.ChoosePeopleToNotifyAddress + "?id=123",
