...
func TestPostCheckYourLpaWhenValidationErrors(t *testing.T) {
	...
	template.
		On("Func", w, mock.MatchedBy(func(data *checkYourLpaData) bool {
			return assert.Equal(t, validation.With("happy", validation.SelectedError{Label: "happyToShareLpa"}), data.Errors)
		})).
		Return(nil)
...
func TestCheckYourLpaFormValidate(t *testing.T) {
	testCases := map[string]struct {
		...
		"invalid-all": {
			...
			errors: validation.
				With("checked", validation.SelectedError{Label: "checkedLpa"}).
				With("happy", validation.SelectedError{Label: "happyToShareLpa"}),
		},
	}
