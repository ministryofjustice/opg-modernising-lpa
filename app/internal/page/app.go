...
func App(
	...
	rootMux.HandleFunc("/lpa/", func(w http.ResponseWriter, r *http.Request) {
		...
		*r2.URL = *r.URL
		r2.URL.Path = path
		if len(r.URL.RawPath) > 0 { // test this works/is needed
			r2.URL.RawPath = r.URL.RawPath[6+len(id)+len(path):]
		}

		lpaMux.ServeHTTP(w, r2.WithContext(contextWithSessionData(r2.Context(), &sessionData{LpaID: id})))
	})
