...
func LoginCallback(tmpl template.Template, oneLoginClient page.OneLoginClient, sessionStore sesh.Store, lpaStore page.LpaStore) page.Handler {
	return func(appData page.AppData, w http.ResponseWriter, r *http.Request) error {
		if r.Method == http.MethodPost {
			certificateProviderSession, err := sesh.GetCertificateProviderSession(sessionStore, r)
			if err != nil {
				return err
			}
		...
		oneLoginSession, err := sesh.GetOneLoginSession(sessionStore, r)
		if err != nil {
			return err
		}
		...
		if !userData.OK {
			...
			if err := sesh.SetCertificateProviderSession(sessionStore, r, w, &sesh.CertificateProviderSession{
				Sub:            userInfo.Sub,
